...
let cookieYouth = $.getdata('youthheader_zq1');
let ARTBODYs = $.getdata('read_zq1');
let READTIME = $.getdata('readtime_zq1');
...
if(!$.isNode()&&cookieYouth.indexOf("#") ==-1){
  ...
  for (let acIdx of zqAc) {
    ...
    cookieArr.push($.getdata(`youthheader_zq1${idx}`));
    readArr.push($.getdata(`read_zq1${idx}`));
    timeArr.push($.getdata(`readtime_zq1${idx}`));
  }
} else {
    if($.isNode()){
        ...
        $.post(kdHost('WebApi/NewTaskIos/getSign'), async(error, resp, data) => {
            ...
            if (signinfo.status == 1) {
               ...
               if(cookieArr.length ==1){
                $.setdata(nick,"zq_nick1")
               } else {
                $.setdata("账号"+cookieArr.length+"合一","zq_nick1")
               }
...
function GetCookie(){if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/NewTaskIos\/getTaskList/)){RefererVal=$request.headers.Referer;signheaderVal=RefererVal.match(/&uid=\d+/)+RefererVal.match(/&cookie=[_a-zA-Z0-9-]+/)+RefererVal.match(/&cookie_id=[a-zA-Z0-9]+/);if(signheaderVal)$.setdata(signheaderVal,'youthheader_zq1');$.log(`${$.name}获取Cookie: 成功, signheaderVal: $}`);$.msg($.name,`获取Cookie: 成功🎉`,``)}else if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/article\/complete/)){const articlebodyVal=$request.body;if(articlebodyVal)$.setdata(articlebodyVal,'read_zq1');$.log(`${$.name}获取阅读: 成功, articlebodyVal: ${articlebodyVal}`);$.msg($.name,`获取阅读请求: 成功🎉`,``)}else if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/v5\/user\/stay/)){const timebodyVal=$request.body;if(timebodyVal)$.setdata(timebodyVal,'readtime_zq1');$.log(`${$.name}获取阅读时长: 成功, timebodyVal: ${timebodyVal}`);$.msg($.name,`获取阅读时长: 成功🎉`,``)}else if($request&&$request.method!=`OPTIONS`&&$request.url.match(/\/withdraw\d?\.json/)){const withdrawVal=$request.body;const withdrawUrl=$request.url;if(withdrawVal)$.setdata(withdrawVal,'cashbody_zq');if(withdrawUrl)$.setdata(withdrawUrl,'cashurl_zq');$.log(`${$.name}, 获取提现请求: 成功, withdrawUrl: ${withdrawUrl}`);$.log(`${$.name}, 获取提现请求: 成功, withdrawBody: ${withdrawVal}`);$.msg($.name,`获取提现请求: 成功🎉`,``)}}
